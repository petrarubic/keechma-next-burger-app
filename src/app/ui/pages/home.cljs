...
(defclassified AddButton :button "focus:outline-none bg-gray-600 hover:bg-gray-700 text-xl text-white font-bold py-2 px-4 border border-gray-700 rounded outline-none mr-5 disabled:opacity-50 disabled:cursor-not-allowed")
(defclassified RemoveButton :button "focus:outline-none bg-gray-600 hover:bg-gray-700 text-xl text-white font-bold py-2 px-4 border border-gray-700 rounded outline-none disabled:opacity-50 disabled:cursor-not-allowed")
...
(defnc Salad [{:keys [count price]}]
  (d/div {:class "slide-in-animation m-auto h-12 w-3/6 mb-2 mt-2 inline-block rounded-full bg-gradient-to-r from-lime-600 to-lime-500"}))

(defnc Bacon [{:keys [count price]}]
  (d/div {:class "slide-out-animation m-auto h-5 w-3/6 mb-2 mt-2 bg-gradient-to-r from-red-700 to-red-600"}))

(defnc Cheese [{:keys [count price]}]
  (d/div {:class "slide-in-animation m-auto h-8 w-3/6 mb-2 mt-2 rounded-full bg-gradient-to-r from-yellow-500 to-yellow-400"}))

(defnc Meat [{:keys [count price]}]
  (d/div {:class "slide-out-animation m-auto h-12 w-3/6 mb-2 mt-2 rounded-lg bg-gradient-to-r from-yellow-900 to-yellow-800"}))
...
(defnc HomeRenderer [props]
  (let [burger-ingredients (use-sub props :burger-builder)
    ...
    ($ HomepageWrapper
       ($ Navbar)
       (d/div {:class "h-full w-full flex flex-col justify-center overflow-hidden"}
              ($ BreadTop)
